...
// Keep these exported so other modules can use them as fallback defaults
...
export const setLocalStorage = () => {
  ...
  if (!localStorage.getItem(ADMIN_KEY)) {
    localStorage.setItem(ADMIN_KEY, JSON.stringify(initialAdmin));
  }
};

export const getLocalStorage = () => {
  if (typeof window === 'undefined') return { employees: null, admin: null };
  try {
    const employees = JSON.parse(localStorage.getItem(EMP_KEY));
    const admin = JSON.parse(localStorage.getItem(ADMIN_KEY));
    return { employees, admin };
  } catch (err) {
    console.error('getLocalStorage parse error', err);
